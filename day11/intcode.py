...
class IntcodeComputer:
    ...
    def _get_value(self, pos: int, mode: int) -> int:
        if mode == 0:
            # pointer mode
            logging.debug(
                f"pos: {pos}, mode: {mode}, value: {self.program[self.program[pos]]}"
            )
            ...
            # relative mode
            logging.debug(
                f"pos: {pos}, mode: {mode}, value: {self.program[self.program[pos] + self.relative_base]}"
            )
    ...
    def _loc(self, pos: int, mode: int) -> int:
        if mode == 0:
            ...
            # relative mode
            logging.debug(
                f"pos: {pos}, mode: {mode}, value: {self.program[pos] + self.relative_base}"
            )
    ...
    def __call__(self, input_values: Iterable[int] = ()) -> int:
        ...
        while True:
            ...
            logging.debug(
                f"pos: {self.pos}, inputs: {self.inputs}, relative_base: {self.relative_base}"
            )
